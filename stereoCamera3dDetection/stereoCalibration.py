...
frame_width = 1280
frame_height = 720
stereo_calibration_frames = 8
view_resize = 2
checkerboard_box_size_scale = 3.9
checkerboard_rows = 4
checkerboard_columns = 6

#get frames NOTE: images must be placed in frame_pairs folder in format cam0_1.png
cam0_images = []
i = 0
while(i < stereo_calibration_frames):
    cam0_images.append(cv.imread("frame_pairs/cam" + str(0) + "_" + str(i + 1) + ".png"))
    i += 1

cam1_images = []
i = 0
while(i < stereo_calibration_frames):
    cam0_images.append(cv.imread("frame_pairs/cam" + str(1) + "_" + str(i + 1) + ".png"))
    i += 1
...
def calibrateSingleCam(images):
    ...
    for frame in images:
        ...
        if ret == True:
            ...
            #opencv can attempt to improve the checkerboard coordinates
            corners = cv.cornerSubPix(gray, corners, conv_size, (-1, -1), criteria)
            cv.drawChessboardCorners(frame, (checkerboard_rows,checkerboard_columns), corners, ret)
            cv.imshow('img', frame)
            k = cv.waitKey(500)
    ...
    print(dist) #Distortion coefficents
    print(rvecs)
    print(tvecs)
    return mtx, dist

def calibrateStereoCam(mtx1, dist1, mtx2, dist2):
    #change this if stereo calibration not good.
    criteria = (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, 100, 0.0001)
    ...
    for frame1, frame2 in zip(cam0_images, cam1_images):
        gray1 = cv.cvtColor(frame1, cv.COLOR_BGR2GRAY)
        gray2 = cv.cvtColor(frame2, cv.COLOR_BGR2GRAY)
        c_ret1, corners1 = cv.findChessboardCorners(gray1, (5, 8), None)
        c_ret2, corners2 = cv.findChessboardCorners(gray2, (5, 8), None)
    ...
    stereocalibration_flags = cv.CALIB_FIX_INTRINSIC
    ret, CM1, dist1, CM2, dist2, R, T, E, F = cv.stereoCalibrate(objpoints, imgpoints_left, imgpoints_right, mtx1, dist1,
    mtx2, dist2, (frame_width, frame_height), criteria = criteria, flags = stereocalibration_flags)

    print(ret)
    return R, T

#calibrate using frame pairs (bc im too lazy to get real calibration frames)
mtx0, dist0 = calibrateSingleCam(cam0_images)
mtx1, dist1 = calibrateSingleCam(cam1_images)

R, T = calibrateStereoCam(mtx0, dist0, mtx1, dist1)
'''